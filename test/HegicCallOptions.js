...
contract('HegicCallOptions', ([user1, user2, user3]) => {
  ...
  it("Should create an option", async () => {
    ...
    const [total, settlementFee] = await CALL.fees(period, amount, price).then(x => [x.total, x.settlementFee])
    ...
    assert(new BN(settlementFee).eq(createEvent.settlementFee), "Wrong settlementFee value")
    assert(new BN(amount).div(new BN(100)).eq(createEvent.settlementFee), "Wrong settlementFee value")
  })
  ...
  it("Shouldn't exercise a new option", async () => {
    ...
    await CALL.exercise(0).then(
      ...
      x => {
        assert.equal(x.reason, 'Option has not been activated yet', 'Wrong error reason')
      }
    )
  })
  ...
  it("Should exercise an option after 15 minutes", async () => {
    ...
    assert.equal(exersizeEvent.id.toNumber(), 0,"Wrong option ID has been initialized")
  })
  ...
  it("Shouldn't unlock an active option", async () => {
    ...
    await CALL.unlock(1).then(
      () => assert.fail("Exercising a call option should be canceled"),
      x => {
        assert.equal(x.reason, 'Option has not expired yet', 'Wrong error reason')
      }
    )
  })
