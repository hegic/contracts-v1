...
contract('HegicCallOptions (Price)', accounts => {
  ...
  const currentPricePromise = contracts.then(({PriceProvider}) => PriceProvider.latestAnswer() )
  for(let i = 0; i < pricePoints.length -1; i++)
    it(`Should have a price for ${pricePoints[i]}% strike greater than the price for ${pricePoints[i+1]}% strike`, async () => {
      ...
      const firstPrice = currentPrice.mul(new BN(pricePoints[i])).div(new BN(100))
      const secondPrice = currentPrice.mul(new BN(pricePoints[i+1])).div(new BN(100))
...
contract('HegicPutOptions (Price)', accounts => {
  ...
  const currentPricePromise = contracts.then(({PriceProvider}) => PriceProvider.latestAnswer() )
  for(let i = 0; i < pricePoints.length -1; i++)
    it(`Should have a price for ${pricePoints[i]}% strike lower than the price for ${pricePoints[i+1]}% strike`, async () => {
