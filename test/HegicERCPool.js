...
contract('HegicERCPool', ([user1, user2, user3]) => {
  ...
  it('Should mint tokens for the first provider correctly', async () =>{
      ...
      assert(
        firstProvide.eq(await ERCPool.shareOf(user1)),
        "Wrong amount"
      )
  })
  ...
  it('Should mint tokens for the second provider correctly', async () =>{
      ...
      assert(
        secondProvide.eq( await ERCPool.shareOf(user2) ),
        "Wrong amount"
      )
  })
  ...
  it('Should distribute the profits correctly', async () => {
    ...
    const expected1 = profit.mul(startShare1).div(startShare1.add(startShare2)).add(startShare1)
    const expected2 = profit.mul(startShare2).div(startShare1.add(startShare2)).add(startShare2)
  ...
  it("Should burn the first provider's tokens correctly", async () => {
    ...
    assert(balanceDelta.eq(value), 'The first provider has received an incorrect amount')
    assert.equal(share1.add(value).sub(startShare1), 0, 'The first provider has an incorrect share')
  })
})
